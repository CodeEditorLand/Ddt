...
fn flow<'a, LI, TI>(
	...
	for (i, func) in last.enumerate() {
		let key = Frame { function:func, depth:shared_depth + i };
	...
	while this.peek().is_some() {
		...
		if let Some(frame_time) = tmp.insert(key, frame_time) {
			unreachable!("start time {} already registered for frame", frame_time.start_time);
		}
...
where
	...
	for line in lines {
		...
		if last.is_empty() {
			...
			flow(&mut tmp, &mut frames, iter::once("").chain(last.split(';')), this, time, delta);
		}
	...
	if !last.is_empty() {
		...
		flow(&mut tmp, &mut frames, iter::once("").chain(last.split(';')), None, time, delta);
	}
