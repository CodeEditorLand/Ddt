...
pub(crate) fn to_collapsed(stacks_file:&Path) -> Result<Vec<u8>, Error> {
	let output =
		OpenOptions::new().read(true).write(false).open(stacks_file).with_context(|| {
			format!("failed to open stacks file ({}) generated by dtrace", stacks_file.display())
		})?;
	...
	Folder::from(collapse_options)
		...
		.with_context(|| {
			format!("unable to collapse generated profile data from {}", stacks_file.display())
		})?;
