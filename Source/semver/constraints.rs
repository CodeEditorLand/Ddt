...
impl ConstraintStorage {
	pub fn root() -> Self {
		Self { cur:Default::default(), parent:Default::default(), children:Default::default() }
	}

	pub fn new(parent:Arc<ConstraintStorage>) -> Self {
		Self { cur:Default::default(), parent:Some(parent), children:Default::default() }
	}

	pub(crate) fn freeze(self) -> Arc<ConstraintStorage> { Arc::new(self) }
...
impl ConstraintStorage {
	pub fn get(&self, name:&PackageName) -> Option<&VersionReq> {
		self.cur.get(name).or_else(|| self.parent.as_ref().and_then(|p| p.get(name)))
	}
